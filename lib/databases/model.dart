import 'package:drift/drift.dart';
import 'package:drift_flutter/drift_flutter.dart';
import 'package:path_provider/path_provider.dart';

import 'package:flutter/material.dart' show TimeOfDay;

part 'model.g.dart';

class RadioStation extends Table {
  /// Outdoor Aerial 服务器分配给广播电台的 UUID
  late final uuid = blob()();

  /// 广播电台的 LOGO，将显示在播放器专辑、封面、首页中，和广播电台官方的宣发图一致
  late final logo = blob()();

  /// 广播电台的横幅，将用于标识节目卡片归属，需要透明 PNG 格式，内容从 LOGO 中选取非文字部分
  late final banner = blob()();

  /// 广播电台的频率，存储单位为 “兆赫兹”，比如 “90.8” 指 90.8MHz
  late final frequency = real()();

  /// 该广播电台的名称，比如 “安徽交通广播”
  late final name = text()();

  /// 对该广播电台的长文本介绍（可选）
  late final description = text()();

  /// 该广播电台所属单位，比如 “安徽广播电视台”
  late final institution = text()();

  /// 该广播电台的播出语言，指定存储 ISO 标准地区代码方便解析，比如 “zh_CN”
  late final language = text()();

  /// 该广播电台拥有的社媒账号，点击就能跳转到指定平台（可选）
  late final social = text()();

  /// 用户是否喜爱该广播电台
  late final like = boolean()();

  /// 广播电台的开播时间，比如广播电台在5点开台，此处就填写5时0分。采用 [TimeOfDay] 存储时间

  /// 广播电台的晚安时间，比如广播电台在23点晚安，此处就填写23时0分。采用 [TimeOfDay] 存储时间

  /// 广播电台的类型，采用 [StationType] 区分类型，比如交通、新闻、经济、综合等
  late final type = intEnum<StationType>()();

  /// 广播电台当前的播出状态，采用 [StationStatus] 标识状态，可标记为 “广播中”、“停机检修” 和 “停播”
  late final status = intEnum<StationStatus>()();
}

@DriftDatabase(tables: [RadioStation])
class ClientDatabase extends _$ClientDatabase {
  ClientDatabase([QueryExecutor? executor]) : super(executor ?? _openConnection());

  @override
  int get schemaVersion => 1;

  static QueryExecutor _openConnection() {
    return driftDatabase(
      name: 'outdoor_aerial_client',
      native: const DriftNativeOptions(
        databaseDirectory: getApplicationSupportDirectory,
      ),
    );
  }
}
